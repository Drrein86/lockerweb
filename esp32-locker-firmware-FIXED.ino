...
// 🛜 הגדרות WiFi
//const char* password = "Neo0528792753";
//String targetPrefix = "neoray_home";

const char* password = "0508882403";
String targetPrefix = "Elior 5g";

// 🌐 הגדרות WebSocket - שרת החומרה ב-Railway
//const char* websocket_host = "locker-ws-server-production.up.railway.app";
...
void webSocketEvent(WStype_t type, uint8_t* payload, size_t length) {
  ...
  switch (type) {
    case WStype_CONNECTED: {
      ...
      cellA1["locked"] = true;
      cellA1["opened"] = false;      // 🔧 תיקון: הוספת opened
      cellA1["hasPackage"] = false;  // 🔧 תיקון: הוספת hasPackage
    ...
    case WStype_TEXT: {
      ...
      // 🔧 תיקון: טיפול בהודעת unlock עם תמיכה הן ב-cell והן ב-cellId
      if (msgType == "unlock") {
        // אם יש ID - נבדוק התאמה, אם אין - נמשיך
        if (targetId != "" && targetId != deviceId) {
          Serial.printf("⚠️ התעלמות מהודעת unlock: ID לא תואם (%s != %s)\n", targetId.c_str(), deviceId.c_str());
          return;
        }

        // 🔧 תיקון: תמיכה הן ב-cellId והן ב-cell
        String cell = "";
        ...
        if (cell != "") {
          ...
          Serial.printf("[%lu] ⚠️ חסר cellId/cell בהודעת unlock\n", millis());
        }
      }
...
bool unlockCell(String cell) {
  ...
  if (!opened) {
    ...
    failDoc["id"] = deviceId;
    failDoc["cellId"] = cell;  // 🔧 תיקון: שימוש ב-cellId במקום cell
  ...
  while (true) {
    ...
    doc["type"] = "cellClosed";
    doc["id"] = deviceId;
    doc["cellId"] = cell;  // 🔧 תיקון: שימוש ב-cellId במקום cell (עדיין תמיכה גם ב-cell בשרת)
    doc["cell"] = cell;    // 🔧 תיקון: הוספת גם cell לתאימות לאחור
...
void setup() {
  ...
  // 🔧 תיקון: בדיקה שהמזהה במתחם המורשה
  Serial.printf("📋 בדיקת מזהה: %s (צריך להיות LOC632 או LOC720)\n", deviceId.c_str());
